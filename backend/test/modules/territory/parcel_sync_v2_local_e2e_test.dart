...
void main() {
  test(
    ...
    () async {
      ...
      await expectLater(
        createTestSyncCoordinator(databaseA, lossy).synchronize(ownerId),
        throwsA(
      ...
      await createTestSyncCoordinator(databaseA, lossy).synchronize(ownerId);
      expect(
        (await databaseA.select(databaseA.syncOutbox).getSingle()).state,
        'done',
      );
      ...
      await createTestSyncCoordinator(databaseB, real).synchronize(ownerId);
