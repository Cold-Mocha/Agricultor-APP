...
Catálogo global de sólo lectura, sembrado por migración y empaquetado en Drift.
...
| `asset_key` | Clave de SVG local aprobada. |
...
Existe como máximo una asignación no eliminada con `effective_to IS NULL` por sector. Cambiar cultivo cierra la vigente y crea otra dentro de una transacción; nunca modifica la asignación histórica de una labor.
...
| `labor_type` | `irrigation`, `soil`, `fertilization`, `disease_pest_control`, `sowing`, `pruning`, `harvest` o `apiary`. |
...
labors     1 ── 0..1 irrigation_records 1 ── 1 irrigation_estimates
...
- Único parcial de asignación vigente `(owner_id, sector_id) WHERE effective_to IS NULL AND deleted_at IS NULL`.
...
`assignment active -> ended`; cambiar cultivo crea otra `active` en la misma transacción.
...
- `crop_catalog` usa IDs/códigos estables idénticos en asset local y seed remoto.
